...
#define AES_CBC false

// Enable/Disable CFB mode
#define AES_CFB false

// Enable/Disable OFB mode
#define AES_OFB false

// Enable/Disable CTR mode
#define AES_CTR false
...
uint8_t decryptedText[INPUT_SIZE] = {0};
...
*/
void encrypt(uint8_t *pText, uint8_t *cText)
{
  ...
  if (debugMode) {
    ...
    ecb_decrypt(cText, decryptedText, INPUT_SIZE);

    // Print encrypted result
    Serial.println("Encryption result:");
    Serial.println((char*) cText);
...
void ecb_encrypt(uint8_t *plainText, uint8_t *cipherText, uint32_t size) {
  ...
  for (block_count = 0; block_count < input_block_size; block_count++) {
    ...
    aes_cipher(plainText + (block_count * AES_BLOCK_SIZE), cipherText + (block_count * AES_BLOCK_SIZE));
  }
}
...
void ecb_decrypt(uint8_t *cipherText, uint8_t *plainText, uint32_t size)
{
  ...
  for (block_count = 0; block_count < input_block_size; block_count++) {
    ...
    aes_inverse_cipher(cipherText + (block_count * AES_BLOCK_SIZE), plainText + (block_count * AES_BLOCK_SIZE));
  }
}
...
/*! \brief Performs random number of random accesses on the sbox
    \param dummmy variable with the result of the previous dummy operation
*/
long randNumber;

uint8_t dummy_operation(uint8_t dummy) {
  while ((randNumber % 2) == 0) {

    randNumber = random(0, 1000);
    dummy = sbox[randNumber % 128];
  }
...
void loop() {
  if (Serial.available () > 0) {
    input_line = Serial.readStringUntil('\n');

    char charBuf[17];
    input_line.toCharArray(charBuf, 17);
